...
internal final class TestValues {
    ...
    static let editBase = (taskSource.data as! ToDoSource).edit(
        label: "Task Source Edited"
        , description: nil
        , category: nil
        , pauses: nil
        , currentTask: (task.data as! ToDoTask)
        , scheduled: nil
        , completed: apr_1_2001
    )
    
    static let taskEdited = AnyTask(editBase.newTask)
    static let taskSourceEdited = AnyTaskSource(editBase.source)
